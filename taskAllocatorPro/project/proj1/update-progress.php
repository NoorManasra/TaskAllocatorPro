...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['task_id'], $_POST['action'])) {
    ...
    try {
        $query = "UPDATE task_team_assignments SET status = :action WHERE task_id = :task_id AND user_id = :user_id";
        
        $stmt = $pdo->prepare($query);
        $stmt->execute([':task_id' => $task_id, ':user_id' => $user_id, ':action' => $action]);

        header("Location: update-progress.php?success=Task status updated.");
        exit();
    } catch (PDOException $e) {
        header("Location: update-progress.php?error=Database error: " . $e->getMessage());
        exit();
    }
}
...
try {
    ...
    WHERE ta.user_id = :user_id AND ta.status = 'Accepted'
    ...
    if (empty($tasks)) {
        $error = "No accepted tasks available.";
    }
} catch (PDOException $e) {
